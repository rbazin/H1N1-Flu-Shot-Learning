...
def train_autofeat_model(data, labels, numerical_list, categorical_list, featen_steps=3, save_file="autofeatModel.pkl"):
    ...
    # Ne peut malheureusement pas classer deux labels en même temps : faire deux algorithmes
    ...
    data_numerical_scaled = scaler.fit_transform(
        data_numerical_imputed.to_numpy())
    data_numerical_scaled = pd.DataFrame(
        data_numerical_scaled, columns=numerical_list)

    # Merging data
    data_imputed_scaled = pd.merge(
        data_numerical_scaled, data_categorical_imputed, left_index=True, right_index=True)

    # Autofeat classifier
    model = AutoFeatClassifier(
        categorical_cols=categorical_list, feateng_steps=featen_steps, verbose=1)
...
def generate_test_features(data, numerical_list, categorical_list, save_file="autofeatModel.pkl"):
    ...
    # Est ce qu'il faut donner un nom d'attribut aux nouvelles features ?
    new_features = model.transform(data_imputed_scaled)

    # On ajoute la colonne respondent Id
    new_features = pd.merge(respondent_id, new_features,
                            left_index=True, right_index=True)

    # On sauvegarde les nouvelles données dans un fichier csv
    new_features.to_csv("new_features_test.csv", sep=",",
